...
public class MethodHandleExample {
    ...
    public void demo1() throws Throwable {
        ...
        // interface method:
        MethodHandle MH_subSequence = publicLookup().findVirtual(CharSequence.class, "subSequence",
                methodType(CharSequence.class, int.class, int.class));
    ...
    public MethodHandle findMethodHandle(String command) throws NoSuchMethodException, IllegalAccessException {
        ...
        if ("DOG".equalsIgnoreCase(command)) {
            mh = lookup.findVirtual(this.getClass(), "bark", voidType);
        } else if ("CAT".equalsIgnoreCase(command)) {
            mh = lookup.findVirtual(this.getClass(), "mewaoo", voidType);
        } else {
